...
public class ListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<TouristPlace> placeList = listOperations.add(place);
		...
		if(add != null) {
			...
			request.setAttribute("bucketList", placeList);
		...
		if(sortbydestination!=null) {
			...
			request.setAttribute("bucketList", listOperations.sortByDestination(placeList));
			RequestDispatcher rd=this.getServletContext().getRequestDispatcher("/WEB-INF/views/list.jsp");
			rd.forward(request, response);
		}
		...
		// call the sortByRank method and store the value in a list variable
			request.setAttribute("bucketList", listOperations.sortByRank(placeList));
			RequestDispatcher rd=this.getServletContext().getRequestDispatcher("/WEB-INF/views/list.jsp");
			rd.forward(request, response);
		}
		...
		if(reset!=null) {	
			...
			request.setAttribute("bucketList", listOperations.reset(placeList));
			RequestDispatcher rd=this.getServletContext().getRequestDispatcher("/WEB-INF/views/list.jsp");
			rd.forward(request, response);
		}
